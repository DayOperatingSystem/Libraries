...
day_buffer_t* construct_day_buffer_from(size_t size, void* memory, void (*flush)(day_buffer_t* buffer), DAYBUFFER_FLUSHMODE mode)
{
	if(size == 0)
		return NULL;
...
day_buffer_t* construct_day_buffer(size_t size, void (*flush)(day_buffer_t* buffer), DAYBUFFER_FLUSHMODE mode)
{
	...
	return construct_day_buffer_from(size, NULL, flush, mode);
}
