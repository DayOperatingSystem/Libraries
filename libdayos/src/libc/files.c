...
int fgetc(FILE* stream)
{
	...
	switch(stream->buffer_mode)
	{
		...
		case _IOFBF:
			...
			{
				stream->buffer_content_size = fread(stream->buffer, stream->buffer_size, 1, stream);
				// debug_printf("Buffer holds: %s %d\n", stream->buffer, stream->buffer_content_size);
				...
				//reverse_memory(stream->buffer, stream->buffer + stream->buffer_content_size);
				//debug_printf("Now Buffer holds: %s\n", stream->buffer);
			...
			if(stream->buffer_content_size != 0 && stream->buffer_index < stream->buffer_size)
			{
...
int fseek(FILE* stream, long int offset, int origin)
{
	...
	// Reset buffer so the right data is going to be
	// fetched next time the stream is read
	stream->buffer_content_size = 0;
...
{
	return !stream || stream->eof || (stream->native_file.offset >= stream->stat.st_size && stream->buffer_content_size == 0);
}
