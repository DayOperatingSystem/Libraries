...
#define ASSERT(msg, expression) if (!tt_assert(__FILENAME__, __LINE__, (msg), (#expression), (expression) ? 1 : 0)) return
...
#define ASSERT_GREATER(expected, actual) ASSERT((#actual), (expected) < (actual))
#define ASSERT_SMALLER(expected, actual) ASSERT((#actual), (expected) > (actual))

#define ASSERT_STRING_EQUALS(expected, actual) ASSERT((actual), strcmp((expected),(actual)) == 0)
//#define ASSERT_STRING_EQUALS(expected, actual) if (!tt_assert(__FILENAME__, __LINE__, (actual), (expected), (!strcmp((expected), (actual))) ? 1 : 0)) return
